...
class ICICIInterest:
    ...
    def parse_tenure(self,tenureText=None):
        print(tenureText)
    ...
    def start(self):
        ...
        #Fetch Fixed Deposit Interests
        r = requests.get(self._BANK_FD_URL, headers=self.headers)
        soup = BeautifulSoup(r.text.encode('utf-8'), "html.parser")
        effectDate = soup.find("div", class_="interest-rates").find('h2').text.replace(
            'FD Interest Rate for General and Senior Citizens(w.e.f. from ', '').replace(')', '')
        from_year = datetime.datetime.strptime(effectDate, "%b %d, %Y").strftime("%d-%m-%Y")
        r = requests.get(self._INTEREST_FD_URL, headers=self.headers)
        filetext = self.removecomments(r.text).replace('\t', '').replace('  ', '').replace('\n', '').replace('\r', '').replace("'",
                                                                                                                          '"')
        ...
        if interestDataDict is not None:
            for inner_list in interestDataDict:
                for entry in inner_list:
                    ...
                    from_period, from_period_d_m_y, from_period_operator, to_period, to_period_d_m_y, to_period_operator = self.parse_tenure(
                        values[0])
                    ...
                    each_row = [self._BANK_NAME, 'NEW', 'FD', 'Regular Fixed Deposit', from_year, from_period, from_period_d_m_y,
                                from_period_operator, '',
                                to_period, to_period_d_m_y,
                                to_period_operator, min_amount, max_amount, 'Y', values[1],
                                float(values[1]) + float(self._senior_rate_inc)]
                    self.appendtocsv(each_row)
                    each_row = [self._BANK_NAME, 'NEW', 'FD', 'Regular Fixed Deposit', from_year, from_period, from_period_d_m_y,
                                from_period_operator, '',
                                to_period, to_period_d_m_y,
                                to_period_operator, min_amount, max_amount, 'Y', values[2],
                                float(values[2]) + float(self._senior_rate_inc)]
                    self.appendtocsv(each_row)
                    each_row = [self._BANK_NAME, 'NEW', 'FD', 'Regular Fixed Deposit', from_year, from_period, from_period_d_m_y,
                                from_period_operator, '',
                                to_period, to_period_d_m_y,
                                to_period_operator, min_amount, max_amount, 'N', values[3],
                                float(values[3]) + float(self._senior_rate_inc)]
                    self.appendtocsv(each_row)
                    each_row = [self._BANK_NAME, 'NEW', 'FD', 'Regular Fixed Deposit', from_year, from_period, from_period_d_m_y,
                                from_period_operator, '',
                                to_period, to_period_d_m_y,
                                to_period_operator, min_amount, max_amount, 'N', values[4],
                                float(values[4]) + float(self._senior_rate_inc)]
                    self.appendtocsv(each_row)
                numList += 1
        #Fetch Recurring Deposit Interests
        r = requests.get(self._BANK_RD_URL, headers=self.headers)
        soup = BeautifulSoup(r.text.encode('utf-8'), "html.parser")
        ...
        for tr in all_trs:
            ...
            each_row = [self._BANK_NAME, 'NEW', 'RD', 'Regular Recurring Deposit', from_year, from_period, from_period_d_m_y,
                        from_period_operator, '',
                        to_period, to_period_d_m_y,
