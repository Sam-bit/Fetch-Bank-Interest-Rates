...
if interestDataDict is not None:
    for inner_list in interestDataDict:
        for entry in inner_list:
            ...
            from_period, from_period_d_m_y, from_period_operator, to_period, to_period_d_m_y, to_period_operator = parse_tenure(
                values[0])
...
for tr in all_trs:
    ...
    appendtocsv(each_row)
each_row = [_BANK_NAME, 'NEW', 'SB', 'Regular Saving Deposit', '04-06-2020', '', '',
            '', '',
            '', '',
            '', 1, 5000000, '', 3, 3]
appendtocsv(each_row)
